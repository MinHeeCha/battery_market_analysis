...
_NUMBER_PATTERN = re.compile(
    r'\d+\.?\d*\s*(%|억|조|달러|만|GWh|GW|위안|원|건|개|명|배|회)|'
    r'\d{4}년|\d+위'
)
...
class ReportEvaluator:
    ...
    def evaluate_layer1(self, report_data: Dict[str, Any]) -> EvaluationResult:
        ...
        # 5. 섹션별 수치 데이터 (각 섹션당 최소 1개)
        sections_to_check = {
            "market_background": "2장",
            "lges_analysis": "3장",
            "catl_analysis": "4장",
            "strategy_comparison": "5장",
        }
        for key, label in sections_to_check.items():
            section_text = json.dumps(report_data.get(key, {}), ensure_ascii=False)
            matches = _NUMBER_PATTERN.findall(section_text)
            details[f"{key}_numbers"] = len(matches)
            if len(matches) < 1:
                failures.append(f"{label}: 수치/통계 데이터 없음 (섹션당 최소 1개 필요)")

        # 6. Comparative SWOT 최소 8개 항목 (S/W/O/T × 2개사)
        ...
        swot_hit = sum(1 for kw in swot_keywords if kw in swot_text)
